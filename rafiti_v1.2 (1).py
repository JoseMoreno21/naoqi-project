...
#!------------------------------------------------------------------------------------
    ...
    if(len(sintomas) != 0):
        tts_service.say("Por favor, necesito que me brindes tu numero de celular numero por numero para mandarte un mensaje con el reporte de tus sintomas")
        num_array=[]
        while len(num_array) != 9:
            tts_recognition_service.pause(False)
            if len(num_array) == 0:
                tts_service.say("Brindame el primer numero")
            if len(num_array) > 0 and len(num_array) < 8:
                tts_service.say("Brindame el siguiente numero")
            if len(num_array) == 8:
                tts_service.say("Brindame el ultimo numero")     
            vocabulary_num=['uno', 'dos', 'tres', 'cuatro', 'cinco', 'seis', 'siete','ocho','nueve']
            tts_recognition_service.setVocabulary(vocabulary_num,True)
            tts_recognition_service.subcribe("numero")
            memory_service.subscribeToEvent('WordRecognized',"numero",'wordRecognized')
            numero_arr=memory_service.getData("WordRecognized")
            if str(numero_arr[0]) in vocabulary_num :
                tts_service.say("Entiendo tu numero es:" +str(numero_arr[0]))
                print("sintoma: %s" % numero_arr[0])
                num_array.append(str(numero_arr[0]))
                tts_recognition_service.unsubscribe("numero")
                memory_service.unsubscribeToEvent('WordRecognized',"numero",'wordRecognized')

                tts_recognition_service.pause(False) #reinicia el servicio de reconocimiento de voz
                boolean=True
            else:
                tts_service.say("No he entendido tu numero puedes repetirlo")
            time.sleep(5)    
        time.sleep(2)
